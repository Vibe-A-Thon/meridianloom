...
| **Version** | **2.1 — GUI-first build, pluggable agent adapters, Python-first / LLM-minimal execution** |
...
| **Scale** | 33 modules · 9 SDLC phases · ~510 numbered requirements |
...
Three stated requirements, each with a module of its own and amendments across the existing thirty:

| Requirement | Module | Principle |
|---|---|---|
| **Build the entire interface first**, then the controlling layer, then everything else | **M32 Simulation Core** — a real sidecar the GUI is built against, with zero model calls | P18 |
| **Every agent is a pluggable, portable adapter** — prebuilt, custom, or an existing agent the organisation already has — with its learned skills travelling with it | **M31 Agent Adapter Framework** | P16, P19 |
| **Python does the heavy lifting; minimal dependency on the model** | **M33 Deterministic Engine** and a rewritten Model Router contract | P17 |

Every existing ID is preserved. Amended requirements are marked `[AMENDED v2.1]`. Changes are listed in §16.5.
...
- **Agent Adapter Framework** — every agent as a self-contained, hot-pluggable package; bridge adapters for agents the organisation has already built (MCP, A2A, LangGraph, CrewAI, OpenAI Agents, plain Python)
- **Instruction files** (`AGENTS.md`, `CONVENTIONS.md`, supporting Markdown) as first-class, versioned, ledger-referenced artifacts
- **Portable learned state** — everything self-evolution produces lives inside the adapter and travels with it
- **Deterministic Engine** — Python-owned capabilities that execute before, and instead of, any model call; a model call must justify itself
- **Simulation Core** — the interface's backend during the GUI-first build, and the golden-corpus and fault-injection harness thereafter
...
| **Agent Adapter** | A self-contained package (manifest, entry point, skills, instructions, `learned/`, probation, tests) that plugs any agent into Meridian Loom through one protocol. Prebuilt agents are adapters too. |
| **Bridge Adapter** | An adapter that wraps an agent built elsewhere — an MCP server, A2A endpoint, LangGraph graph, CrewAI crew, OpenAI Agent, or a Python callable — translating it to the `AgentAdapter` protocol without rewriting it. |
| **Instruction File** | `AGENTS.md`, `CONVENTIONS.md` or any supporting Markdown an agent works from. Versioned, human-editable, ledger-referenced on every invocation. |
| **Learned State** | The `learned/` directory of an adapter: promoted policy versions, distilled rules and templates, owned procedural memory, calibration history. The only place self-evolution writes. |
| **Action Class** | The category of an agent action (parse, resolve, test, scaffold, design, review, …) that determines whether it is deterministic, assisted, or generative. |
| **Deterministic / Assisted / Generative** | Deterministic: Python only, no model. Assisted: Python does the work, the model fills a bounded gap. Generative: the model produces, Python validates. |
| **LLM Dependency Ratio** | Model-produced actions ÷ total actions, per agent, phase, and story. A KPI expected to fall over time. |
| **Simulation Core** | A real sidecar implementing the full message-bus contract, producing real ledger entries and loop state from cassette replay and deterministic simulators, with zero model calls. |
...
| **P16** | **Every agent is an adapter.** | Prebuilt, custom, and bridged agents plug in through one protocol, are discovered from a folder, and are added or removed without redeploying the extension or restarting the sidecar. The prebuilt agents are built as adapters so the framework is proven by the product's own workforce. |
| **P17** | **Python first, model last.** | Every action class has a deterministic path unless it genuinely requires judgement. The router refuses a model call where a deterministic path exists, and every permitted call records why it was needed. The Trainer's best output is a rule that removes a model call. |
| **P18** | **The interface is built first, against a real backend.** | Not fixtures — the Simulation Core: real contracts, real ledger, real loop state, replayed reasoning. The GUI is built once and does not change when live agents arrive. |
| **P19** | **Learning is data inside the adapter.** | Self-evolution writes only to `learned/`. Export is packaging the folder. What an agent learned travels with it, and nothing it learned is stranded in the runtime. |
...
└── Meridian Core (Python 3.11+) ◄───┘   [local host, or the Remote host]
    ...
    ├── Agent Adapter Framework — discovery, validation, probation, hot plug/unplug,
    │                             bridges (MCP · A2A · LangGraph · CrewAI · OpenAI Agents · Python)
    ├── Instruction Library — AGENTS.md / CONVENTIONS.md / *.md, versioned and ledger-referenced
    ├── Deterministic Engine — Python-owned action classes; runs before any model call
    ├── Model Router — called last; tiering, caching, redaction, failover, backpressure,
    │                  refuses calls that have a deterministic path, records why_llm
    ...
    ├── Simulation Core — cassette replay + deterministic story simulators
...
├── adapters/                 # workspace-local agent adapters, one folder each (committed)
│   └── <adapter-id>/
│       ├── adapter.yaml      # §7.9
│       ├── agent.py          # AgentAdapter implementation or bridge
│       ├── skills/  instructions/  probation/  tests/
│       └── learned/          # the only directory self-evolution writes (committed or synced)
├── instructions/             # workspace-level AGENTS.md, CONVENTIONS.md, *.md (committed)
├── simulation/               # story simulators and scripted scenarios for the Simulation Core
└── exports/                  # agent export packages
```

User-level adapters live in `~/.meridian/adapters/` and organisation-level adapters in the registry (`FR-M16-08`); discovery order is workspace → user → organisation, and an adapter id resolves to the nearest.
...
Thirty-three modules, in numeric order, grouped by the concern each serves. Numbering follows source-document order and is stable.

| Family | Modules |
|---|---|
| **Platform & runtime** | M1 Extension Host · M2 Webview Dashboard · M3 Sidecar & IPC · M4 Loop Runtime · M27 Replay & Runtime Testing · M30 Runtime Operations · **M32 Simulation Core** |
| **The agent organisation** | M5 Agent Registry · M6 Skill Loader · M7 Memory Fabric · M15 Onboarding · M16 Replicator · **M31 Agent Adapter Framework** |
| **Intelligence & economics** | M8 Model Router · M26 Cost Forecasting · **M33 Deterministic Engine** |
...
| ID | Requirement | Priority |
|---|---|---|
| FR-M5-01 | **[AMENDED v2.1]** Every agent SHALL be defined by a versioned manifest (schema in §7.1), never by hard-coded source. **The manifest is a facet of the adapter manifest (§7.9); the registry SHALL be populated by adapter discovery (`FR-M31-02`), not by a built-in list.** | MUST v1 |
...
| FR-M5-07 | The registry SHALL distinguish adapter provenance — `prebuilt`, `custom`, `bridged` — and SHALL surface it on every agent, since governance is identical but trust history is not. | MUST v1 |
...
| FR-M6-10 | Skill packs SHALL be installable standalone **or shipped inside an adapter's `skills/` directory**; an adapter-shipped pack is bound to that adapter by default and MAY be shared with others by policy. | MUST v1 |
| FR-M6-11 | Skill packs acquired by an agent through self-evolution SHALL be written to that adapter's `learned/` directory as versioned packs, and SHALL travel with the adapter on export (`FR-M31-12`). | MUST v1.x |
...
| FR-M7-14 | **Instruction Library.** `AGENTS.md`, `CONVENTIONS.md`, and any `*.md` under `.meridian/instructions/` or an adapter's `instructions/` SHALL be first-class artifacts: discovered, versioned, diffable, human-editable, and included in context assembly by declared precedence (adapter → workspace → user → organisation). | MUST v1 |
| FR-M7-15 | Every invocation SHALL record in the ledger the digest of each instruction file that entered its context, so a change in an agent's behaviour can be traced to a change in its instructions. | MUST v1 |
| FR-M7-16 | Instruction files SHALL be treated as trusted only when they originate from the workspace, user, or organisation tiers under version control; instruction-like content found in repository or story text is untrusted per `FR-M7-07`. | MUST v1 |
...
| FR-M8-15 | **[v2.1 — Python first, model last]** The router SHALL refuse a model call for any action class the Deterministic Engine (M33) declares it can satisfy, unless policy explicitly permits the call for that class. Refusals SHALL be ledger-recorded. | MUST v1 |
| FR-M8-16 | Every permitted model call SHALL carry a `why_llm` reason (`no_deterministic_path` · `assisted_gap` · `generative_by_policy` · `human_override`) recorded in the ledger entry. | MUST v1 |
| FR-M8-17 | The router SHALL expose the **LLM dependency ratio** per agent, phase, story, and action class (`FR-M17-10`), and SHALL support a policy ceiling on it that escalates when breached. | MUST v1 |
| FR-M8-18 | For *assisted* action classes the router SHALL prefer the smallest model tier that satisfies the schema, including local models (`FR-M8-10`), before any frontier tier. | SHOULD v1 |
...
| FR-M14-14 | **[v2.1]** The Trainer SHALL write only to the target adapter's `learned/` directory — never to the adapter's `agent.py`, its manifest, or the runtime. Promotion is a versioned change to `learned/policy/`. | MUST v1.x |
| FR-M14-15 | **Distillation to rules.** Where the Trainer detects a recurring model-produced pattern with stable outcomes, it SHALL propose a deterministic rule or template into `learned/rules/`, evaluated and promoted under the same gates, so that the action class moves from generative or assisted to deterministic (`FR-M33-06`). | MUST v1.x |
| FR-M14-16 | The Trainer SHALL operate identically on prebuilt, custom, and bridged adapters, subject to each adapter's declared `trainable:` surfaces (§7.9). An adapter declaring no trainable surface is never modified. | MUST v1.x |
| FR-M14-17 | For a bridged agent that carries its own learning mechanism, policy SHALL decide (`D17`) whether that mechanism runs, is suspended, or runs in shadow with Meridian's Trainer authoritative. The choice SHALL be recorded in the adapter manifest and the ledger. | MUST v1.x |
...
| ID | Requirement | Priority |
|---|---|---|
| FR-M15-01 | **[AMENDED v2.1]** A new agent role SHALL be addable via manifest and wizard **without redeploying or modifying the extension** — and, as of v2.1, **without restarting the sidecar**: onboarding is the human-facing path onto `FR-M31-04` hot plug. | MUST v1.x |
...
| ID | Requirement | Priority |
|---|---|---|
| FR-M16-01 | **[AMENDED v2.1]** Export SHALL produce a portable package containing: agent manifest, policy and prompt set, skill bindings and digests, selected procedural memory, evaluation results, and provenance metadata. **As of v2.1 the package IS the adapter folder (§7.9), including the entire `learned/` directory, so that everything the agent acquired travels with it.** | MUST v1.x |
...
| FR-M17-10 | **[v2.1] LLM dependency ratio** — model-produced actions ÷ total actions — per agent, phase, story, and action class, with its trend over time and the count of action classes moved to deterministic by distillation (`FR-M14-15`). | MUST v1 |
...
### M31 — Agent Adapter Framework *(new in v2.1)*

*The agent organisation.* One protocol for every agent — prebuilt, custom, or already built elsewhere. **The prebuilt agents in §6.10 are implemented as adapters**, so this framework is exercised by the product's own workforce from P2 onward.

| ID | Requirement | Priority |
|---|---|---|
| FR-M31-01 | An **`AgentAdapter` protocol** SHALL be defined in Python with these methods: `describe()` → capabilities, action classes and tool needs; `plan(task, context)`; `act(step, context)`; `reflect(outcome)`; `learn(signal)`; `export_state()` / `import_state()` over `learned/`; `health()`; `probation_tasks()`. The protocol is versioned and the version is declared in `adapter.yaml`. | MUST v1 |
| FR-M31-02 | The framework SHALL **discover** adapters from `.meridian/adapters/`, `~/.meridian/adapters/`, and the organisation registry, in that precedence, and SHALL populate the Agent Registry (M5) from discovery. Discovery SHALL run on sidecar start and on filesystem change. | MUST v1 |
| FR-M31-03 | Discovery SHALL **validate** each adapter: manifest schema, protocol version compatibility, declared tools present in the policy allow-list, skill and instruction files resolvable, signature and digest where present. An invalid adapter SHALL be listed with its validation errors and SHALL NOT be loaded. | MUST v1 |
| FR-M31-04 | Adapters SHALL be **hot-pluggable**: adding a folder loads and admits the adapter into probation, and removing it retires the agent — **with no extension redeploy and no sidecar restart**. In-flight work owned by an unplugged adapter SHALL be checkpointed and escalated, never lost. | MUST v1 |
| FR-M31-05 | **Bridge adapters** SHALL be provided for: an **MCP server** exposing agent tools; an **A2A** endpoint (agent card); a **LangGraph** compiled graph; a **CrewAI** agent or crew; an **OpenAI Agents SDK** agent; and a **plain Python callable** with a declared signature. Which bridges ship in v1 is `D15`; MCP and plain-Python are MUST v1. | MUST v1 (MCP, Python) / SHOULD v1.x (rest) |
| FR-M31-06 | A bridge SHALL translate to the `AgentAdapter` protocol **without modifying the wrapped agent**, SHALL run the wrapped agent inside the sandbox of `FR-M9-05`, and SHALL route every tool call the wrapped agent attempts through the Tool Layer's permission check (`FR-M9-03`). **A bridged agent cannot bypass governance by virtue of being external.** | MUST v1 |
| FR-M31-07 | Every adapter — prebuilt, custom, bridged — SHALL enter through **probation** (`FR-M15-03`) at the `suggest` tier and SHALL be subject to the same autonomy-tier rules. Prebuilt adapters MAY ship with a pre-recorded probation result that is re-verified on first load. | MUST v1 |
| FR-M31-08 | An adapter SHALL declare in its manifest which surfaces are **trainable** (`policy`, `rules`, `memory`, `skills`, `calibration`) and which are frozen. The Trainer (M14) SHALL honour the declaration. | MUST v1 |
| FR-M31-09 | An adapter SHALL declare the **action classes** it handles and, for each, whether it handles them deterministically, assisted, or generatively (§7.10). The Deterministic Engine (M33) uses this to route. | MUST v1 |
| FR-M31-10 | Adapters are executable code and SHALL be governed as skill packs are: reviewed on install with every file, script and URL disclosed (`FR-M6-06`), digest-pinned, sandboxed, tool-scoped, revocable (`SEC-19`), and signed where the source is external (`SEC-24`). | MUST v1 |
| FR-M31-11 | An **adapter SDK** SHALL be published — a Python package with the protocol, a scaffold command (`meridian adapter new`), a local test harness that runs an adapter against the Simulation Core, and a conformance suite an adapter must pass before it can be signed. | MUST v1.x |
| FR-M31-12 | **Portability.** An adapter's `learned/` directory SHALL be the complete record of what the agent has acquired; `export_state()` SHALL produce it and `import_state()` SHALL consume it, and export (`FR-M16-01`) SHALL package the whole adapter folder. | MUST v1 |
| FR-M31-13 | Two adapters MAY fill the same role concurrently (e.g., the prebuilt Developer and a bridged in-house Developer); policy SHALL define selection — by stack, by trust score, by explicit assignment, or by shadow comparison (`FR-M25-07`). | SHOULD v1.x |
| FR-M31-14 | Adapter versions SHALL be immutable and upgrades SHALL follow `FR-M16-09`: the new version runs the adapter's own `tests/` and the golden corpus before it replaces the old. | MUST v1.x |
| FR-M31-15 | An adapter SHALL be able to declare **dependencies on other adapters** (e.g., a Refactoring adapter requiring a Verification adapter); the framework SHALL resolve and validate the dependency graph at discovery. | SHOULD v1.x |

### M32 — Simulation Core *(new in v2.1)*

*Platform & runtime.* **The resolution of P18.** The interface is built first, in full, against this — a real sidecar with real contracts and a real ledger, whose reasoning is replayed or simulated rather than generated.

| ID | Requirement | Priority |
|---|---|---|
| FR-M32-01 | The Simulation Core SHALL implement the **complete message-bus contract** of the production sidecar — every request, response and subscription — so that a webview built against it requires no changes when the control layer arrives. | MUST v1 |
| FR-M32-02 | It SHALL produce **real ledger entries** through the real Ledger Service (M10): hash-chained, Merkle-indexed, signed. Simulated entries carry `replay_of` or a `simulated` marker and SHALL never be confused with live work. | MUST v1 |
| FR-M32-03 | It SHALL drive **real loop-state transitions** through the Loop Runtime's state machine (M4) — the six loops, bounds, checkpoints, interrupts and gates — with the agent-reasoning step satisfied by cassette replay (M27) or by a deterministic simulator. | MUST v1 |
| FR-M32-04 | It SHALL ship **scripted story scenarios** covering every screen's data needs: a clean story, a story with rework and unravels, a story hitting a loop bound, a blocked gate, a clarifying question, a steer, a multi-story portfolio, a tamper-detected ledger, a budget breach, an adapter plug and unplug. | MUST v1 |
| FR-M32-05 | It SHALL make **zero model calls**. Any code path that would call a model in the Simulation Core is a defect. | MUST v1 |
| FR-M32-06 | It SHALL support **time control** — pause, step, play at N×, jump to sequence — so the GUI's time-travel (`FR-M4-07`) and motion are developed against controllable state. | MUST v1 |
| FR-M32-07 | On the arrival of the control layer, the Simulation Core SHALL become the **golden-corpus runner** (`FR-M27-03`) and the **fault-injection harness** (`FR-M27-04`). It is never discarded. | MUST v1 |
| FR-M32-08 | The adapter SDK's local harness (`FR-M31-11`) SHALL run an adapter against the Simulation Core, so a custom agent can be developed and tested with no live model and no live repository. | MUST v1.x |
| FR-M32-09 | A **contract-drift check** SHALL run in CI: the Simulation Core's contract and the production sidecar's contract are the same generated types, and a divergence fails the build. | MUST v1 |

### M33 — Deterministic Engine *(new in v2.1)*

*Intelligence & economics.* **The resolution of P17.** Python owns every capability that does not require judgement; the model is the last resort and must say why it was needed.

| ID | Requirement | Priority |
|---|---|---|
| FR-M33-01 | An **action-class catalogue** (§7.10) SHALL be maintained in policy, classifying every agent action as `deterministic`, `assisted`, or `generative`, with the owning engine and the conditions under which the class may escalate to a model call. | MUST v1 |
| FR-M33-02 | The engine SHALL implement, in Python and without any model call: structural parsing and edits (tree-sitter); symbol resolution and references (LSP); running builds, tests, linters and scanners; diff and patch; scaffolding from skill-pack templates; dependency, license and migration-reversibility classification; blast-radius from graph analysis; work-packet decomposition from the dependency graph; coverage-to-criteria mapping; cost estimation from ledger history; rule-based ambiguity detection; and convention checks from `learned/rules/`. | MUST v1 |
| FR-M33-03 | Every agent action SHALL be **dispatched to the deterministic path first**. Only when the engine reports `no_deterministic_path` for that action class, or the class is `assisted`/`generative` by policy, MAY the router (`FR-M8-15`) be invoked. | MUST v1 |
| FR-M33-04 | For `assisted` classes the engine SHALL do the substantive work and hand the model a **bounded gap** with a schema — never the whole task — and SHALL validate the model's return against the schema and against deterministic checks before accepting it. | MUST v1 |
| FR-M33-05 | For `generative` classes the engine SHALL **validate every model output** deterministically before it takes effect: syntax, types, tests, conventions, scope. A model output that fails validation is rework, not a result. | MUST v1 |
| FR-M33-06 | The engine SHALL load **distilled rules and templates** from every active adapter's `learned/rules/` and SHALL apply them ahead of any model call for the matching action class, so that Trainer distillation (`FR-M14-15`) directly reduces model dependency. | MUST v1.x |
| FR-M33-07 | The engine SHALL report per action class: deterministic hit rate, assisted rate, generative rate, and the latency and cost saved versus the generative path. These feed `FR-M17-10`. | MUST v1 |
| FR-M33-08 | The engine's deterministic paths SHALL be **replay-identical** — same inputs, same outputs — so that stories run through them are reproducible without cassettes. | MUST v1 |
| FR-M33-09 | Policy SHALL be able to set a **model-call ceiling** per story, phase and adapter (count and spend), independent of the token budget, breach of which escalates rather than proceeds. | MUST v1 |
| FR-M33-10 | Reclassifying an action class (e.g., from `assisted` to `deterministic` once a rule proves itself) SHALL be a policy change — reviewed, ledger-recorded, reversible — never an automatic runtime decision (`D16`). | MUST v1.x |
...
**[v2.1]** Every agent in this roster is implemented as an **adapter** (M31). The roster names roles; adapters fill them. The organisation's existing agents fill roles through bridge adapters alongside the prebuilt ones, and policy selects between adapters for the same role (`FR-M31-13`).
...
### 7.9 Adapter Manifest *(new — M31)*

```yaml
adapter:
  id: acme-java-developer          # unique across the registry
  version: 2.3.0                   # immutable per release
  protocol: 1                      # AgentAdapter protocol version
  provenance: bridged              # prebuilt | custom | bridged
  bridge: langgraph                # only when bridged: mcp | a2a | langgraph | crewai | openai-agents | python
  entry: agent.py:AcmeJavaDeveloper
  signature: sha256:…              # required for external sources (SEC-24)

role:
  fills: Developer                 # a role from §6.10, or a new role
  tier: L2
  phases: [BD]
  stacks: [java-spring-gradle, java-fullstack]

capabilities:
  action_classes:                  # §7.10 — what this adapter handles, and how
    scaffold:      deterministic
    implement:     generative
    unit_test:     assisted
    convention_fix: deterministic  # served from learned/rules
  tools: [repo.read, repo.patch, build.invoke, test.invoke, lsp.query, ast.parse]
  skills: [java-spring-gradle]     # shipped in ./skills or resolved from the catalogue
  instructions: [instructions/AGENTS.md, instructions/CONVENTIONS.md]
  depends_on: []                   # other adapter ids (FR-M31-15)

learning:
  trainable: [policy, rules, memory, calibration]   # surfaces the Trainer may write
  frozen: [skills]
  own_mechanism: shadow            # bridged agents only: run | suspend | shadow  (D17)

governance:
  autonomy_tier: suggest           # always on admission
  budgets: { tokens_per_packet: 300000, cost_usd_per_packet: 4.00, wallclock_minutes: 30 }
  model_call_ceiling: { per_packet: 12 }               # FR-M33-09
  confidence_thresholds: { design: 0.70, implement: 0.60, unit_test: 0.65 }
  escalation_target: build-orchestrator
  probation: { task_set: probation/, pass_threshold: 0.85 }
```

### 7.10 Action Class Policy *(new — M33)*

```yaml
action_classes:
  parse:            { mode: deterministic, engine: tree-sitter }
  resolve_symbol:   { mode: deterministic, engine: lsp }
  run_tests:        { mode: deterministic, engine: tool-layer }
  scaffold:         { mode: deterministic, engine: skill-templates }
  classify_migration: { mode: deterministic, engine: rules }
  blast_radius:     { mode: deterministic, engine: graph }
  decompose_packets: { mode: deterministic, engine: graph, escalate_if: cyclic_or_ambiguous }
  estimate_cost:    { mode: deterministic, engine: ledger-history }
  detect_ambiguity: { mode: assisted, deterministic_first: rules, gap: "propose resolution", schema: AmbiguityProposal }
  draft_adr:        { mode: assisted, deterministic_first: outline, gap: "prose sections", schema: ADR }
  name_tests:       { mode: assisted, schema: TestNames }
  implement:        { mode: generative, validate: [syntax, types, tests, conventions, scope] }
  design_novel:     { mode: generative, validate: [adr_schema, nfr_allocation] }
  clarify:          { mode: generative, validate: [question_schema] }
  rationale:        { mode: generative, validate: [label_unverified] }
ceilings:
  llm_dependency_ratio_max: 0.35   # per story; breach escalates
```
...
| NFR-24 | **Hot plug latency**: an adapter added to `.meridian/adapters/` SHALL be discovered, validated and visible in the roster (in probation) within 5 seconds, with no sidecar restart. | MUST v1 |
| NFR-25 | **LLM dependency**: on the golden corpus, the LLM dependency ratio SHALL be ≤ 0.35 at P2 acceptance and SHALL not rise between releases without a recorded policy decision. | MUST v1 |
| NFR-26 | **Simulation parity**: every message-bus message the production sidecar emits SHALL be producible by the Simulation Core; verified by the contract-drift check on every build. | MUST v1 |
| NFR-27 | **Deterministic reproducibility**: any story that touches only deterministic action classes SHALL replay byte-identically without a cassette. | MUST v1 |
...
| SEC-24 | **Adapters are executable code.** Every adapter from a source other than the extension itself SHALL be signed, digest-pinned, reviewed on install with full disclosure, sandboxed, tool-scoped, and revocable. An unsigned external adapter SHALL NOT load. | MUST v1 |
| SEC-25 | **Bridge isolation.** A bridged agent SHALL run inside the sandbox, SHALL have no network egress except through the Tool Layer's allow-list, SHALL have no credential access, and SHALL be unable to invoke any tool the bridge has not declared and policy has not permitted. A bridged agent that attempts to bypass the Tool Layer SHALL be paused and the attempt ledger-recorded. | MUST v1 |
| SEC-26 | **Learned state is data, never code.** `learned/rules/` and `learned/policy/` SHALL contain only declarative rules, templates and prompts in a validated schema; the engine SHALL refuse to load executable content from `learned/`. | MUST v1 |
...
| AC-23 | **[v2.1]** A custom adapter folder dropped into `.meridian/adapters/` appears in the roster in probation within 5 s, with no sidecar restart; removing the folder retires the agent and its ledger history remains. |
| AC-24 | **[v2.1]** An existing agent wrapped by the plain-Python bridge completes a work packet under Meridian governance — every tool call permission-checked, every action ledger-recorded — with the wrapped code unmodified. |
| AC-25 | **[v2.1]** An adapter exported after learning a team convention, then imported into a fresh workspace, applies that convention on its first packet from `learned/rules/` with no model call. |
| AC-26 | **[v2.1]** On the reference story, every scaffold, parse, test-run, migration-classification and blast-radius action executes with zero model calls, and the LLM dependency ratio for the story is ≤ 0.35. |
| AC-27 | **[v2.1]** A model call attempted for an action class with a deterministic path is refused, and the refusal is ledger-recorded. |
| AC-28 | **[v2.1]** The complete webview, built against the Simulation Core, runs unchanged when the production sidecar replaces it — verified by the contract-drift check and by running every screen's scenario on both. |
| AC-29 | **[v2.1]** A bridged agent attempting network egress outside the allow-list is paused, and the attempt is ledger-recorded. |
...
**[v2.1 — re-sequenced for GUI-first.]** The interface is built first, in full, against the Simulation Core (M32), then the control layer, then the adapter framework with the prebuilt agents built as adapters. First value arrives one phase later than in v2.0; in exchange the interface is complete before live reasoning exists and is never rebuilt. Detailed sequencing is in `Requirements-implementation.md` v2.0.

| Phase | Modules | Release | Exit condition |
|---|---|---|---|
| **S0 — Simulation Core & contracts** | M1, M2 (shell), M3 (incl. remote), M10 (substrate, encrypted blobs), M18, M27 (cassettes, fault injection), **M32**, M30 (doctor, migration, uninstall), generated message-bus types | v1 | Every screen's scenario runs on the Simulation Core; contract-drift check green; zero model calls |
| **GUI — G0 … G8** | The entire interface (`viguix-implementation.md`) | v1 | All 44 screens complete against the Simulation Core |
| **C1 — Control layer** | M4, M5, M7 (incl. instruction library), M8 (LLM-minimal contract), M9, M12 (v1 subset), M13, M20 (identity), M25, M26, M28, **M33** | v1 | Live loops drive the existing GUI with no GUI change; `AC-27` |
| **C2 — Adapters & prebuilt agents** *(first value)* | **M31**, the §6.10 roster built as adapters, M6 (single skill), M17 (core), M19 (Jira), M23 (GitHub), M24 (hover + chat), SDLC phases 1–7, one plain-Python bridge for an existing agent | v1 | AC-01 … AC-29 on the reference repository; §10.2 quality bar |
| **C3 — Quality gates** | M12 (full), M6 (full catalogue), M16 (upgrade/revocation), phase 5–7 additions | v1.x | Gates block real defects; three stacks; `NFR-25` held |
| **C4 — Learning & portability** | M14 (per-adapter, writing to `learned/`, distillation), M15, M16 (adapter packages), remaining bridges | v1.x | Trainer raises first-pass yield and moves ≥1 action class to deterministic; an adapter round-trips with its learning |
| **C5 — Orchestration, scale & compliance** | Phase Orchestrators, M21, M22, M23 (rest), M19 (rest), M29, M17 (full), M20 (full), M30 (full), phases 8–9, anchoring, crypto-shredding, tenant isolation | v1.x | Multi-story, multi-repo, audit bundle accepted, erasure completes |
| **C6 — Differentiation** | All v2 items | v2 | — |

The previous table (P0–P5) is retained below for cross-reference; the phase *contents* are unchanged, the *order* is not.

Engineering phases P0–P4 as mapped in v2.0. GUI phases G0–G8 run alongside (`viguix-implementation.md`).
...
| **R21** | **Adapter supply chain — a malicious or careless external adapter** | **Critical** | `SEC-24` signing and review; `SEC-25` bridge isolation; `FR-M31-10`; probation at `suggest`; `SEC-19` revocation |
| **R22** | **Contract drift between the Simulation Core and the production sidecar leaves the GUI-first interface wrong on arrival** | **High** | `FR-M32-09` generated shared types with a CI drift check; `NFR-26`; `AC-28` |
| **R23** | **A bridged agent escapes governance through its own tool or network access** | **Critical** | `FR-M31-06`, `SEC-25` — all tool calls through the Tool Layer, sandbox, no egress; attempts pause the agent |
| **R24** | **"Python first" degrades output quality where a deterministic path is weaker than the model** | **Medium** | Action classes are policy (`FR-M33-01`); reclassification is reviewed (`FR-M33-10`); `FR-M33-07` reports per-class outcomes; shadow comparison (`FR-M25-07`) before reclassifying |
| **R25** | **Learned state carries a poisoned rule from one team to another on export** | **High** | `SEC-26` declarative-only; import enters probation (`FR-M16-05`); `FR-M7-07` untrusted tagging survives export; distilled rules are gate-evaluated before promotion |
...
| **D14** | Whether the Security phase is P1 or P2, and whether `AC-02` should name it — raised in `Requirements-implementation.md` §0.3 | Delivery + Security | C2 start |
| **D15** | Which bridge adapters ship in v1. MCP and plain-Python are MUST; A2A, LangGraph, CrewAI and OpenAI Agents each cost real work and depend on which existing agents the organisation actually has | Architecture + the owners of existing agents | C2 start |
| **D16** | Who owns the action-class catalogue, and the evidence bar for reclassifying a class from assisted to deterministic once a distilled rule proves itself | Governance | C1 |
| **D17** | For a bridged agent with its own learning mechanism: run, suspend, or shadow with Meridian's Trainer authoritative | Governance | C4 |
| **D18** | Where `learned/` is persisted for a workspace adapter — committed to the repository, synced to the organisation registry, or both | Architecture | C2 |
...
### 16.5 Changes in v2.1

**New modules:** M31 Agent Adapter Framework (15 requirements) · M32 Simulation Core (9) · M33 Deterministic Engine (10).

**Amended in place, marked `[AMENDED v2.1]`:** `FR-M5-01` (registry populated by adapter discovery) · `FR-M15-01` (no sidecar restart) · `FR-M16-01` (export is the adapter folder including `learned/`).

**Added to existing modules:** `FR-M5-07` · `FR-M6-10`, `FR-M6-11` · `FR-M7-14`…`16` (Instruction Library) · `FR-M8-15`…`18` (LLM-minimal router contract) · `FR-M14-14`…`17` (Trainer writes only to `learned/`, distillation to rules, bridged-agent learning) · `FR-M17-10` (LLM dependency ratio).

**Added elsewhere:** §0.6 · §1.2 six in-scope items · §2 nine glossary terms · §3 P16–P19 · §4.1 topology · §4.2 `adapters/`, `instructions/`, `simulation/` · §4.3 module map · §6.10 adapter note · §7.9 Adapter Manifest · §7.10 Action Class Policy · NFR-24…27 · SEC-24…26 · AC-23…29 · §12 re-sequenced (S0, GUI, C1–C6) · R21–R25 · D14–D18.

**Superseded but retained:** the v2.0 phase table in §12, kept for cross-reference.
...
| **M31 Agent Adapter Framework** | FR-M31-01 … 15 | 15 |
| **M32 Simulation Core** | FR-M32-01 … 09 | 9 |
| **M33 Deterministic Engine** | FR-M33-01 … 10 | 10 |
| *v2.1 additions to M5, M6, M7, M8, M14, M17* | FR-M5-07 · FR-M6-10…11 · FR-M7-14…16 · FR-M8-15…18 · FR-M14-14…17 · FR-M17-10 | 15 |
| **Module subtotal** | | **332** |
...
| **Phase subtotal** | | **67** |
| Non-functional | NFR-01 … 27 | 27 |
| Security | SEC-01 … 26 | 26 |
| Acceptance criteria | AC-01 … 29 | 29 |
| Ecosystem | ECO-01 … 08 | 8 |
| Risks | R1 … R25 | 25 |
| Open decisions | D1 … D18 | 18 |
| **Total numbered items** | | **533** |
