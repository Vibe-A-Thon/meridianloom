...
const agent: WorkbenchAgent = { id: 'atlas', name: 'Atlas', role: 'Developer', description: 'Portable ACP agent', vendor: 'Custom', version: '1.0.0', command: 'atlas-acp', args: ['--stdio'], instructions: 'Explain verification.', permissions: ['read'], trainable: ['memory'], mode: 'active', runtime: 'idle', learningState: 'waiting', createdAt: '2026-09-08T00:00:00Z', updatedAt: '2026-09-08T00:00:00Z' };
function controller(overrides: Partial<WorkbenchSnapshot> = {}) {
  const snapshot: WorkbenchSnapshot = { revision: 1, agents: [agent], deliverables: [], runs: [], learning: [], capabilities: { workspaceOpen: true, trusted: true, governorEnabled: true, executionReady: true }, ...overrides };
  const execute = vi.fn(async () => snapshot);
  return { controller: { snapshot, execute: execute as WorkbenchExecute, busy: false, error: null, refresh: async () => {} } satisfies WorkbenchController, execute };
}

describe('Agent studio', () => {
  it('sends a validated portable profile from the add form without activation or execution', async () => {
    const h = controller(); render(<AgentStudio controller={h.controller} />);
    ...
    fireEvent.change(screen.getByLabelText('Executable', { exact: false }), { target: { value: 'sage-acp' } });
    fireEvent.change(screen.getByLabelText('Arguments (JSON array)', { exact: false }), { target: { value: '["--stdio"]' } });
    fireEvent.click(screen.getByRole('button', { name: 'Create agent' }));
    await waitFor(() => expect(h.execute).toHaveBeenCalledTimes(1));
    expect(h.execute).toHaveBeenCalledWith('agent/save', { agent: expect.objectContaining({ id: 'sage', name: 'Sage', command: 'sage-acp', args: ['--stdio'], trainable: ['memory'] }) });
  ...
  it('invalid argument JSON stays in the form and never crosses the host boundary', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Agent name'), { target: { value: 'Sage' } });
    fireEvent.change(screen.getByLabelText('Executable', { exact: false }), { target: { value: 'sage-acp' } });
    fireEvent.change(screen.getByLabelText('Arguments (JSON array)', { exact: false }), { target: { value: '{"secret":"oops"}' } });
    fireEvent.click(screen.getByRole('button', { name: 'Create agent' }));
  ...
  it('deactivation requests Learning mode, and host results update the status', async () => {
    const h = controller(); const view = render(<AgentStudio controller={h.controller} />);
    ...
    view.rerender(<AgentStudio controller={{ ...h.controller, snapshot: { ...h.controller.snapshot!, agents: [{ ...agent, mode: 'learning' }] } }} />);
  ...
  it('edits an existing identity and requires a concrete removal dialog', async () => {
    ...
    await waitFor(() => expect(h.execute).toHaveBeenCalledWith('agent/save', { agent: expect.objectContaining({ id: 'atlas', name: 'Atlas 2' }) }));
    await waitFor(() => expect(screen.queryByRole('dialog')).toBeNull());
    fireEvent.click(screen.getByRole('button', { name: 'Inspect Atlas' }));
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Remove agent', exact: true }));
  ...
  it('keeps host failures visible in the add form', async () => {
    const h = controller(); h.execute.mockRejectedValue(new Error('State could not be saved.'));
...
describe('Delivery and Learning studios', () => {
  it('draft creation does not dispatch and shows only the active roster', async () => {
    const h = controller({ agents: [agent, { ...agent, id: 'sage', name: 'Sage', mode: 'learning' }] });
    ...
    fireEvent.change(screen.getByLabelText('Deliverable title'), { target: { value: 'Accessible dialog' } });
    fireEvent.change(screen.getByLabelText('Brief and acceptance criteria'), { target: { value: 'Verify focus restoration.' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save draft' }));
    await waitFor(() => expect(h.execute).toHaveBeenCalledWith('deliverable/save', { title: 'Accessible dialog', brief: 'Verify focus restoration.' }));
    expect(h.execute).toHaveBeenCalledTimes(1);
  });
  it('dispatch is disabled when execution is unavailable, with the reason shown', () => {
    const h = controller({ capabilities: { workspaceOpen: true, trusted: false, governorEnabled: true, executionReady: false, executionBlockedReason: 'Trust the workspace first.' }, deliverables: [{ id: 'brief-1', title: 'Test brief', brief: 'Do the work.', state: 'draft', agentIds: [], createdAt: agent.createdAt, updatedAt: agent.updatedAt }] });
    render(<DeliveryStudio controller={h.controller} />);
  ...
  it('learning review states its source and only sends the explicit accept decision', async () => {
    const h = controller({ agents: [{ ...agent, mode: 'learning', learningState: 'review' }], learning: [{ id: 'note-1', agentId: 'atlas', deliverableId: 'brief-1', title: 'Remember focus', content: 'Return focus after closing.', state: 'pending', createdAt: agent.createdAt, surface: 'memory' }] });
    ...
    await waitFor(() => expect(h.execute).toHaveBeenCalledWith('learning/review', { id: 'note-1', decision: 'accepted' }));
  });
  it('covers all 51 surfaces while clearly separating pending interfaces', () => {
    ...
    fireEvent.change(screen.getByLabelText('Search capability coverage'), { target: { value: 'UML Studio' } });
