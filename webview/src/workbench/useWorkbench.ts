...
export function useWorkbench(client: WebviewRpcClient, ready: boolean): WorkbenchController {
  ...
  useEffect(() => {
    ...
    const unsubscribe = client.addListener((message) => {
      if (
        message.type === 'event' &&
        (message.event.kind === 'workbench/changed' || message.event.kind === 'tiers/changed')
      )
        void refresh();
    });
    return () => {
  ...
  const execute: WorkbenchExecute = useCallback(
    async <A extends WorkbenchAction>(action: A, params: WorkbenchActionMap[A]['params']) => {
