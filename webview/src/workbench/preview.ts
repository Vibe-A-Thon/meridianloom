...
export function createPreviewTransport(): RpcTransport {
  ...
  (window as { acquireVsCodeApi?: unknown }).acquireVsCodeApi = () => ({
    ...
    postMessage: (message: {
      ...
      if (message.type !== "download" || typeof message.content !== "string")
        return;
      const url = URL.createObjectURL(
        new Blob([message.content], { type: message.mimeType ?? "text/plain" }),
      );
      const anchor = document.createElement("a");
      anchor.href = url;
      anchor.download = message.fileName ?? "meridian-export.txt";
      ...
      checks: [
        {
          ...
          remediation:
            "Launch the extension in VS Code for actual diagnostics.",
        },
      ],
    },
  };
          ...
          if (message.method in rpc)
            ...
            receive?.({
              ...
              error: {
                code: -32001,
                message:
                  "This operation needs the real VS Code extension host.",
              },
            });
          return;
        }
        ...
        try {
          ...
          switch (message.action) {
            case "document/save": {
              ...
              if (old && old.version !== input.expectedVersion)
                throw new Error("Document changed. Refresh before saving.");
              ...
              if (old) state.documents![state.documents!.indexOf(old)] = next;
              else state.documents!.unshift(next);
              break;
            }
            case "document/remove": {
              const old = state.documents!.find((d) => d.id === params.id);
              if (!old || old.version !== params.expectedVersion)
                throw new Error("Document changed. Refresh before removing.");
              state.documents = state.documents!.filter(
                (d) => d.id !== params.id,
              );
              state.documentRevisions = state.documentRevisions!.filter(
                (d) => d.id !== params.id,
              );
              break;
            }
            ...
            case "document/import": {
              ...
              if (
                portable.kind !== "meridian-studio-document" ||
                portable.schemaVersion !== 1
              )
                throw new Error("Expected a Meridian studio document.");
            ...
            case "document/restore": {
              const old = state.documents!.find((d) => d.id === params.id);
              const revision = state.documentRevisions!.find(
                (d) => d.id === params.id && d.version === params.version,
              );
              if (!old || !revision || old.version !== params.expectedVersion)
                throw new Error("Revision unavailable or document changed.");
              state.documentRevisions!.push(structuredClone(old));
              state.documents![state.documents!.indexOf(old)] = {
                ...revision,
            ...
            case "agent/remove":
              ...
              state.learning = state.learning.filter(
                (note) => note.agentId !== params.id,
              );
              break;
            case "agent/mode":
            ...
            case "agent/import": {
              ...
              if (
                ...
                throw new Error(
                  "Use a portable agent document with a unique ID.",
                );
              state.agents.push({
                ...document.agent,
                mode: "learning",
            ...
            case "deliverable/complete": {
              ...
              if (!item || item.state !== "review")
                throw new Error(
                  "The sample deliverable is not ready for review.",
                );
