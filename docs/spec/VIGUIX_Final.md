...
| **Version** | **2.1 — GUI-first build, pluggable agent adapters, Python-first / LLM-minimal surfaces** |
...
| **Companions** | `vision.md` (architecture) · `Requirements_Final.md` (engineering, 33 modules) · `viguix-implementation.md` (build order) |
| **Scale** | 44 screens · 26 cross-cutting systems · ~230 numbered interface requirements |
...
Three requirements from `Requirements_Final.md` v2.1 with interface consequences:

| Requirement | Interface consequence |
|---|---|
| **The GUI is built first**, against the Simulation Core (M32) | Every screen must render correctly on simulated state and must say so — a **simulation-mode indicator** (X-26). Nothing in the interface may assume live reasoning exists. |
| **Every agent is a pluggable adapter** (M31) | Two new screens — **10.43 Adapter Bay** and **10.44 Instruction Library** — and provenance (`prebuilt` / `custom` / `bridged`) surfaced on every agent everywhere. |
| **Python first, model last** (M33) | The **action class** and **`why_llm`** of every agent action become visible: a new primitive (`ActionClassChip`), a Crown meter, an Observatory panel, and a Trace-tab section. The interface must make it impossible to not know whether a result came from Python or from a model. |

Every existing ID is preserved. Changes are listed in §22.5.
...
**A fourth audience, added in v2.1.** The **agent author** — someone plugging in an agent the organisation already built, or authoring a new one, who needs to know within a minute whether it loaded, why it failed validation, what it is permitted to do, and what it has learned. 10.43 Adapter Bay exists for them.

**A v2.1 truth the interface must carry.** Because Python does the heavy lifting and the model is the last resort, the interface never lets a user *not know* whether something was computed deterministically or generated. This is not a debugging affordance; it is part of what makes the system's output trustworthy.
...
| **Status Bar** | Loom state, active pass count, live spend, **LLM dependency ratio**, Halt All | Compressed to `⟡ 4 weaving · $2.18 · 0.28 llm` *(v2.1)* |
| **Simulation banner** *(v2.1)* | When the backend is the Simulation Core, a persistent cochineal band under the Crown reading `simulation · scenario: rework-heavy · seq 4402/4417 · no model calls` | X-26. Never dismissible while simulated. |
...
| `ActionClassChip` *(v2.1)* | `deterministic` / `assisted` / `generative` for any agent action, as a shape+colour pair: solid iron square · half-filled woad square · hollow cochineal square. Never colour alone. Carries the `why_llm` reason on hover and focus. |
| `ProvenanceTag` *(v2.1)* | `prebuilt` / `custom` / `bridged (langgraph)` on every agent, everywhere an `AgentToken` appears at 28px or larger. |
| `LLMRatioMeter` *(v2.1)* | A small meridian arc showing model-produced actions ÷ total, with the policy ceiling marked and the trend arrow. Lives in the Status Bar, the Crown, and 10.22. |
| `AdapterCard` *(v2.1)* | Adapter identity, provenance, protocol version, validation state, role filled, action-class summary, learned-state size, trainable surfaces, signature status. |
| `InstructionFileCard` *(v2.1)* | Path, tier (adapter / workspace / user / org), digest, last change, which agents consumed it in the last N invocations. |
...
| v2.1 additions | | |
|---|---|---|
| E-CC-08 | **LLM dependency tile** beside Spend: `LLMRatioMeter` for this story, with the count of model calls refused because a deterministic path existed (`FR-M8-15`). Clicking opens 10.31. | MUST v1 |
| E-CC-09 | Every pass row carries an `ActionClassChip`, so the live list shows at a glance which passes are Python and which are the model. | MUST v1 |
| E-CC-10 | **Simulation banner** (X-26) when the backend is the Simulation Core. The Command Center is the first screen built against it and the first place the indicator must be right. | MUST v1 |
...
| E-AW-06 *(v2.1)* | **Provenance column** (`ProvenanceTag`) and **LLM ratio column** per agent. A bridged agent's bridge type is shown; an agent with a rising ratio is flagged. Row action: **Open in Adapter Bay**. | MUST v1 |
...
| E-DJ-06 *(v2.1)* | **Distillation pane** (`FR-M14-15`): recurring model-produced patterns the Trainer proposes to turn into deterministic rules or templates, each with its evidence count, the action class it would move from *assisted* or *generative* to *deterministic*, and the model calls per story it would remove. Promotion writes to `learned/rules/`, never to code (`SEC-26`). | MUST v1.x |
| E-DJ-07 *(v2.1)* | **Trainable surfaces** shown per adapter from its manifest (`policy` · `rules` · `memory` · `skills` · `calibration`), with frozen surfaces greyed and the reason. A bridged agent's own learning mechanism state (`run` / `suspend` / `shadow`, `D17`) is shown beside them. | MUST v1.x |
| E-DJ-08 *(v2.1)* | The candidate graph's nodes carry `ActionClassChip`s, so the effect of a candidate on model dependency is visible in the topology, not only in the numbers. | SHOULD v1.x |
...
| E-SF-04 *(v2.1)* | Skill cards show **which adapter ships the pack** (`FR-M6-10`) and whether it is shared or adapter-bound; **learned skill packs** (`FR-M6-11`) render with a cochineal dotted border and link to the adapter's `learned/` in 10.43. | MUST v1 |
...
| E-OB-04 *(v2.1)* | The wizard is the human-facing path onto hot plug (`FR-M31-04`). Step 1 gains a choice — **New role** (author a manifest) or **Plug in an existing agent** (choose a folder or a bridge type, then the wizard fills the manifest from `describe()`). Both paths end in the same probation step. | MUST v1 |
| E-OB-05 *(v2.1)* | Bridge selection shows what the bridge will and will not do in plain language — "Your LangGraph graph runs unmodified inside Meridian's sandbox. Every tool call it makes is permission-checked. It cannot reach the network except through the allow-list." (`FR-M31-06`, `SEC-25`). | MUST v1 |
...
| E-IN-07 *(v2.1)* | Trace tab: every step carries an `ActionClassChip`; model-produced steps show their `why_llm` reason (`FR-M8-16`); refused model calls (`FR-M8-15`) appear as a distinct row. The header shows `ProvenanceTag` and a link to the adapter in 10.43. | MUST v1 |
| E-IN-08 *(v2.1)* | Memory tab gains a **Learned** section — the adapter's `learned/` contents by surface (policy, rules, memory, calibration) with sizes and last-promoted dates, read-only here, editable in 10.43. | MUST v1 |
| E-IN-09 *(v2.1)* | New **Instructions** section on the Trace tab: the instruction files that entered this invocation's context with their digests (`FR-M7-15`), each linking to 10.44. | MUST v1 |
...
| E-KP-06 *(v2.1)* | **LLM dependency ratio** (`FR-M17-10`) over time, per agent, phase, story and action class, with the policy ceiling drawn and every reclassification of an action class (`FR-M33-10`) as an annotation event. The expected shape is downward; an upward trend triggers an inline finding. | MUST v1 |
| E-KP-07 *(v2.1)* | **Deterministic-path savings** — latency and cost avoided versus the generative path, per action class (`FR-M33-07`). The panel that justifies the engine to a finance audience. | SHOULD v1 |
...
| E-EX-03 *(v2.1)* | **The export is the adapter folder** (`FR-M16-01`, `FR-M31-12`). The pre-export view shows the folder tree with `learned/` expanded — every rule, policy version and calibration file that will travel — and what the scan strips. Import shows the same tree on arrival. | MUST v1.x |
...
### 10.31 Model Routing Observatory *(S32)* · **MUST v1** *(v2.1 — promoted)*

**Serves:** M8, **M33**. Promoted from SHOULD v1 because the Python-first policy makes this the screen where model dependency is governed.
...
| v2.1 additions | | |
|---|---|---|
| E-RO-01 | **Action-class matrix** (`FR-M33-01`, §7.10): action class × mode (`deterministic` / `assisted` / `generative`) with the owning engine, editable by Governor with the blast-radius preview. Reclassification is a policy change and shows as one (`FR-M33-10`). | MUST v1 |
| E-RO-02 | **Refusal log** (`FR-M8-15`): every model call refused because a deterministic path existed — agent, action class, what the engine did instead. | MUST v1 |
| E-RO-03 | **`why_llm` breakdown** (`FR-M8-16`): permitted model calls by reason — `no_deterministic_path` · `assisted_gap` · `generative_by_policy` · `human_override` — per agent and phase. | MUST v1 |
| E-RO-04 | **Model-call ceilings** (`FR-M33-09`) per story, phase and adapter, with breaches and their escalations. | MUST v1 |
| E-RO-05 | **Distilled rules in effect** (`FR-M33-06`): rules loaded from every active adapter's `learned/rules/`, the action class each serves, and the model calls each has replaced. | MUST v1.x |
...
### 10.43 Adapter Bay *(new in v2.1)* · **MUST v1**

**Serves:** M31. The screen for the fourth audience — the agent author. Everything about plugging an agent in, and everything an agent has become.

```
┌─ INSTALLED ──────────────────────┬─ SELECTED: acme-java-developer ─────────────┐
│ ● prebuilt  developer      2.1.0 │ bridged · langgraph · protocol 1 · v2.3.0     │
│ ● prebuilt  analyst        2.1.0 │ ✓ validated  ✓ signed  ● probation 4/6        │
│ ◐ bridged   acme-java-dev  2.3.0 │ fills Developer · BD · java-spring-gradle     │
│ ○ custom    docs-writer    0.4.1 │ ────────────────────────────────────────────  │
│ ✕ invalid   legacy-tester  1.0.0 │ ACTION CLASSES                                │
│   manifest: protocol 0 unsupported│ scaffold ■ det   implement □ gen   test ◧ ast │
│                                  │ convention_fix ■ det  (from learned/rules)    │
│ ┌─ DROP A FOLDER HERE ─────────┐ │ ────────────────────────────────────────────  │
│ │  or: New adapter · Plug in   │ │ TOOLS  repo.read repo.patch build test lsp   │
│ │  existing · Browse registry  │ │ TRAINABLE  policy rules memory calibration    │
│ └──────────────────────────────┘ │ FROZEN  skills                                │
│                                  │ ────────────────────────────────────────────  │
│                                  │ LEARNED  policy v1.4 · 12 rules · 41 memory   │
│                                  │          calibration 3 classes · 1.2 MB       │
│                                  │ [Unplug] [Export] [Open folder] [Run tests]   │
└──────────────────────────────────┴───────────────────────────────────────────────┘
```

- **Installed list** with provenance glyph (● prebuilt · ◐ bridged · ○ custom · ✕ invalid), version, validation state, and probation progress. **Invalid adapters are listed, not hidden**, with their validation errors in plain language (`FR-M31-03`).
- **Drop target** for a folder (`FR-M31-04` hot plug) plus three actions: *New adapter* (scaffold via the SDK, `FR-M31-11`), *Plug in existing* (opens 10.15 at the bridge step), *Browse registry* (`FR-M16-08`).
- **Selected adapter** — `AdapterCard` expanded: identity, protocol, signature (`SEC-24`), role and stacks, **action classes with `ActionClassChip`s**, tools, trainable and frozen surfaces, dependencies (`FR-M31-15`), and the **Learned** panel with sizes and last-promoted dates per surface.
- **Learned browser** — drill into `learned/policy`, `learned/rules`, `learned/memory`, `learned/calibration` as versioned, diffable files. Rules are read-only here (promotion happens in the Dojo); memory entries link to 10.30.
- **Actions:** Unplug (hold; shows in-flight work that will be checkpointed and escalated), Export (opens 10.23 with the folder tree), Open folder, Run tests (the adapter's own `tests/` against the Simulation Core, `FR-M32-08`), Re-validate.
- **Role conflicts** — when two adapters fill the same role, the selection policy (`FR-M31-13`) is shown and editable here.
- **Live plug animation:** a dropped folder validates with `thread-tension`, then the agent's `portrait-hire` runs and it walks onto the Floor. An unplug walks it off. The screen and the Floor tell the same story.

**Empty state:** "No adapters beyond the prebuilt roster. Drop a folder, or plug in an agent you already have." — with the six bridge types listed.

---

### 10.44 Instruction Library *(new in v2.1)* · **MUST v1**

**Serves:** M7 (`FR-M7-14`…`16`). `AGENTS.md`, `CONVENTIONS.md` and every supporting Markdown that tells agents how the organisation works — as first-class, governed artifacts rather than files that happen to be on disk.

- **Four tiers as columns** with precedence arrows: **adapter → workspace → user → organisation**. An instruction that overrides one further right shows the override.
- **`InstructionFileCard`s** — path, tier, digest, last change and by whom, and **consumption**: which agents pulled this file into context in the last N invocations, with a link to the ledger entries (`FR-M7-15`).
- **Editor** with the X-08 sanitised preview, a diff against the last version, and *Open PR* rather than *Save* for governed tiers (mirrors `E-CF-02`).
- **Trust boundary made visible:** files from the workspace, user and organisation tiers under version control render with a linen border (trusted); instruction-like content detected in repository or story text renders in the untrusted holding area of 10.30 and is linked from here, never listed as an instruction (`FR-M7-16`).
- **Impact preview:** before saving, the screen lists the adapters whose next invocation will include the changed file — the blast-radius mechanism applied to instructions.
- **Search** across all tiers; **stale detection** when a file references code that has changed (`FreshnessBadge`).
...
25. **[v2.1]** Displaying an agent result without its action class. The user must always be able to tell whether Python computed it or a model generated it.
26. **[v2.1]** Any interface that treats a prebuilt agent differently from a custom or bridged one, other than by the `ProvenanceTag`. Governance is identical; the interface must not imply otherwise.
27. **[v2.1]** Simulated state rendered without the simulation band. Rehearsal must never look like reality.
...
| **`D15`** *(v2.1)* | Which bridge types 10.15 and 10.43 offer in v1. Mirrors `Requirements_Final.md` `D15`. | Adapter Bay, Onboarding | G1 |
| **`D16`** *(v2.1)* | Whether the action-class matrix (E-RO-01) is Governor-editable in the interface or policy-file-only. Mirrors `Requirements_Final.md` `D16`. | Routing Observatory | G7 |
| **`V8`** *(v2.1)* | Whether the simulation band (X-26) also appears in Focus Mode, where chrome is otherwise hidden. Recommend yes — rehearsal on a wall screen is the most dangerous place to mistake it for reality. | Focus Mode | G4 |
...
### 22.5 Changes in v2.1

**New screens:** 10.43 Adapter Bay · 10.44 Instruction Library. **New system:** X-26 Simulation mode. **New primitives:** `ActionClassChip`, `ProvenanceTag`, `LLMRatioMeter`, `AdapterCard`, `InstructionFileCard`.

**Added to existing screens:** E-CC-08…10 · E-AW-06 · E-DJ-06…08 · E-SF-04 · E-OB-04, 05 · E-IN-07…09 · E-KP-06, 07 · E-EX-03 · E-RO-01…05 (10.31, promoted to MUST v1).

**Amended:** §0.6 · §1 fourth audience and the deterministic-vs-generated truth · §7.1 Status Bar and simulation banner · §18 banned patterns 25–27 · §19 `D15`, `D16`, `V8` · §23 coverage for M31–M33.
...
| **M31 Agent Adapter Framework** *(v2.1)* | 10.43, 10.15, 10.4, 10.23 | Covered |
| **M32 Simulation Core** *(v2.1)* | X-26, 10.29, 10.1 | Covered |
| **M33 Deterministic Engine** *(v2.1)* | 10.31, 10.22, 10.16, `ActionClassChip` everywhere | Covered |
...
|---|---|---|---|
| Screens | 10.1 … 10.44 | 44 | 24 from v1.0, 18 added in v2.0 (S25–S42; S29 folded into 10.17), **2 added in v2.1** |
| `E-` | E-AR-01 … E-WV-07 (+ v2.1: E-CC-08…10, E-AW-06, E-DJ-06…08, E-SF-04, E-OB-04…05, E-IN-07…09, E-KP-06…07, E-EX-03, E-RO-01…05) | 116 | Enhancements, folded into parent screens |
| `X-` | X-01 … X-26 | 26 | Cross-cutting systems (§11) |
...
| `V` | V1 … V8 | 8 | **Open design decisions** (§19) — not hyphenated |
| `D` | D8, D11, D15, D16 | 4 | Decisions shared with `Requirements_Final.md` |
| **Total numbered items** | | **260** | |
