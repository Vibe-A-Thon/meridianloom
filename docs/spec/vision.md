...
|---|---|
| **Document** | vision.md |
| **Status** | Draft **v1.1** — architecture vision, revised for GUI-first build, pluggable agent adapters, and Python-first / LLM-minimal execution |
...
### 1.3 Three stances added in v1.1

**The interface is built first, in full, before the control layer.** Not against fixtures — against a **Simulation Core**: a real Python sidecar that implements the complete message-bus contract, produces real hash-chained ledger entries, and drives real loop-state transitions from replayed story cassettes and deterministic simulators, with zero live model calls. The GUI is therefore built once, against the real data model, and does not change when live reasoning arrives. The Simulation Core is not thrown away afterwards; it becomes the golden-corpus runner and the fault-injection harness. See §13.

**Agents are adapters.** Every agent in Meridian Loom — the prebuilt roles in §2, the organisation's own already-built agents, and any custom agent authored later — is a **pluggable adapter** conforming to one protocol. Adapters are added and removed like plugins, without redeploying the extension and without restarting the sidecar. The prebuilt agents are built *as* adapters, so the adapter framework is proven by the product's own workforce before a third-party agent ever plugs in. See §2.8.

**Python does the heavy lifting; the model is the last resort.** Every agent action is classified as *deterministic* (Python only), *assisted* (Python does the work, the model fills a bounded gap) or *generative* (the model produces, Python validates). A model call is made only when no deterministic path exists for the action class, and every call records *why* it was needed. The system's **LLM dependency ratio** is a first-class KPI expected to fall over time — partly because the Trainer converts recurring model-produced patterns into templates and rules the deterministic engine can run. See §5.1.
...
### 2.8 Every agent is an adapter

Tiers L0–L4 and the meta-agents describe *what an agent does in the organisation*. The **Agent Adapter** describes *how any agent plugs in*. The two are orthogonal: a Developer role can be filled by the prebuilt adapter, by an adapter wrapping an agent the organisation already built, or by a custom adapter authored next year.

An adapter is a self-contained package:

```
my-agent/
├── adapter.yaml          # identity, role, capabilities, tools, trainable surfaces, entry point
├── agent.py              # implements the AgentAdapter protocol (or a bridge to an existing agent)
├── skills/               # skill packs this agent ships with  (SKILL.md format)
├── instructions/         # AGENTS.md, CONVENTIONS.md, any *.md the agent works from
├── learned/              # everything self-evolution has produced — travels with the agent
│   ├── policy/           # versioned prompt / playbook deltas promoted by the Trainer
│   ├── rules/            # deterministic rules and templates distilled from model output
│   ├── memory/           # procedural memory this agent owns
│   └── calibration/      # confidence-calibration history per action class
├── probation/            # the task set that admits this adapter to live work
└── tests/                # the adapter's own regression suite, run before any promotion
```

**Five properties that follow:**

1. **Plug and unplug.** Drop the folder in, the registry discovers and validates it, probation runs, the agent walks onto the Floor. Remove the folder, the agent is retired and its ledger history is untouched. No extension redeploy, no sidecar restart.
2. **Bridges for existing agents.** An organisation's already-built agents plug in through **bridge adapters** — thin translations from MCP servers, A2A endpoints, LangGraph graphs, CrewAI crews, OpenAI Agents, or a plain Python callable to the `AgentAdapter` protocol. The existing agent is not rewritten; it is wrapped, governed, and recorded like every other agent.
3. **Learning travels with the agent.** Everything self-evolution produces lives in `learned/`. Exporting an agent (§8.2) is packaging the folder. A Java agent that has learned a team's conventions over six months carries those conventions to the next team as data, not as a retraining exercise.
4. **Instructions are first-class artifacts.** `AGENTS.md`, `CONVENTIONS.md` and any supporting Markdown are versioned, human-editable, referenced by the ledger on every invocation, and reviewed like code. They are how an organisation tells an agent how it works here.
5. **Same governance for every adapter.** Prebuilt, bridged or custom, an adapter is executable code: reviewed on install, digest-pinned, sandboxed, tool-scoped, and admitted only through probation at the `suggest` tier. Trust is earned identically.
...
│  ├────────────────────────────────────────────────────────┤ │
│  │ Agent Adapter Framework — discovery, validation,       │ │
│  │ probation, hot plug/unplug, bridges to existing agents │ │
│  ├────────────────────────────────────────────────────────┤ │
│  │ Agent Registry │ Skill Loader │ Instruction Library    │ │
│  │                │              │ Memory Fabric          │ │
│  ├────────────────────────────────────────────────────────┤ │
│  │ Deterministic Engine — parse, resolve, analyse, test,  │ │
│  │ scaffold, classify, estimate: Python before any model  │ │
│  ├────────────────────────────────────────────────────────┤ │
│  │ Model Router — called last; tiering, caching, budget,  │ │
│  │ every call records why a model was needed              │ │
│  ├────────────────────────────────────────────────────────┤ │
│  │ Tool Layer — MCP client, LSP, tree-sitter, build/test  │ │
│  ├────────────────────────────────────────────────────────┤ │
│  │ Simulation Core — cassette replay + deterministic      │ │
│  │ story simulators; the GUI's backend before live agents │ │
│  ├────────────────────────────────────────────────────────┤ │
...
### 5.1 Python first, model last

The brief's requirement — most of the work in Python, the model only for what genuinely needs it — is an architectural commitment, not a preference. The **Deterministic Engine** sits in front of the Model Router and owns every capability that does not require judgement:

| Deterministic in Python | Assisted (Python + bounded model call) | Generative (model, Python validates) |
|---|---|---|
| Parsing and structural edits (tree-sitter) · symbol resolution and references (LSP) · running builds, tests, linters, scanners · diff and patch · scaffolding from skill-pack templates · dependency and license checks · migration reversibility classification · blast-radius from graph analysis · packet decomposition from the dependency graph · coverage-to-criteria mapping · cost estimation from ledger history · rule-based ambiguity detection · convention checks from `learned/rules` | Ambiguity resolution proposals · ADR drafting from a deterministic outline · test case naming and assertion messages · review commentary on a deterministically-found defect · summaries for gates | Novel implementation where no template applies · design reasoning for unfamiliar problems · clarifying questions · rationale narrative (always labelled unverified) |

Three consequences. First, **a model call must justify itself** — the router refuses a call for an action class that has a deterministic path, and every permitted call records a `why_llm` reason in the ledger. Second, **the LLM dependency ratio is a KPI**, and the Trainer's most valuable output is not a better prompt but a rule or template that removes a model call entirely (distilled into `learned/rules`, never into source code). Third, **cost, latency and reproducibility all improve together**, because the deterministic path is free, instant and replayable.
...
**Learning is portable because it is data in the adapter.** Self-evolution never writes outside an adapter's `learned/` directory. Prompts, playbooks, distilled rules and templates, procedural memory and calibration history are all files, versioned, diffable and packaged with the agent. An adapter exported after a year of use arrives at the next team with everything it learned, and a bridged agent that the organisation built elsewhere accumulates the same `learned/` state from the day it is plugged in.
...
Six extension points, each data-driven, none requiring an extension redeploy or a sidecar restart:

1. **Agent Adapters** — any agent, prebuilt or custom or bridged from an existing framework, as a self-contained pluggable package (§2.8)
2. **Skill Packs** — new stack or domain capability (`SKILL.md` folder), shipped inside an adapter or installed standalone
3. **Instruction Files** — `AGENTS.md`, `CONVENTIONS.md` and supporting Markdown that tell agents how the organisation works
4. **Agent Manifests** — new role, onboarded through probation (now a facet of the adapter manifest)
5. **MCP Servers** — new tools and system integrations
6. **Policy Packs** — organisation-specific gates, standards, thresholds, and the deterministic-vs-model classification per action class
...
**v1.1 re-sequencing — the interface first.** The full interface is built against the Simulation Core before the control layer exists. The engineering phases below therefore start with the Simulation Core and contracts, then the entire GUI, then the control layer, then the adapter framework with the prebuilt agents built as adapters. First value — a real story through a real chain — arrives one phase later than in v1.0; in exchange, the interface is complete, tested against real data structures, and never rebuilt.

| Phase | Scope | Proves |
|---|---|---|
| **S0 — Simulation Core** | Message-bus contract, data models, real ledger substrate, cassette replay, deterministic story simulators, Doctor | The GUI has a real backend to build against, with zero model calls |
| **GUI — all phases** | Every screen, against the Simulation Core | The interface is complete and never rebuilt |
...
**The sequencing principle:** prove first-pass yield and unit economics on one stack and one slice before adding a single additional agent. **And in v1.1: the prebuilt agents are the first adapters built, so the adapter framework is proven by the product's own workforce before any external agent plugs in.** Every phase after 1 is an amplifier. Amplifying an unproven core amplifies error.
...
| D7 | Which bridge adapters ship in v1 — MCP and plain-Python are certain; A2A, LangGraph, CrewAI, OpenAI Agents each cost real work | Which existing agents plug in on day one |
| D8 | The deterministic-vs-model classification for each action class — who decides, and how a class is reclassified once a distilled rule proves itself | Where the LLM-dependency line is drawn |
| D9 | Whether a bridged agent's *own* learning mechanism (if it has one) is allowed to run, or is replaced by Meridian's Trainer writing to `learned/` | Governance of externally-built agents |
