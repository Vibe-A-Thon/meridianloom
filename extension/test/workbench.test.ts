...
describe('persisted agent workbench', () => {
  it('creates Learning agents, edits and removes only the selected profile, and reloads from disk', async () => {
    ...
    expect(
      JSON.parse(await readFile(path.join(root, '.meridian/workbench/state.json'), 'utf8')).agents,
    ).toHaveLength(1);
  });
  it('rejects invalid manifests and unsafe IDs without changing the saved state', async () => {
    ...
    await save(service, 'atlas');
    await expect(
      service.request({ action: 'agent/save', params: { agent: { ...input('../escape') } } }),
    ).rejects.toThrow('ID');
    await expect(
      service.request({
        action: 'agent/save',
    ...
    expect(() =>
      validateWorkbenchAgent({ ...input('secret'), args: ['--api-key=private'] }),
    ).toThrow('credentials');
  });
  it('exports a portable profile, imports without activation, and rejects duplicate imports', async () => {
    ...
    await expect(
      two.service.request({ action: 'agent/import', params: { content: document.content } }),
  ...
  it.each(['trust', 'governor', 'sidecar'] as const)(
    ...
    async (blocked) => {
      ...
      await activate(service, 'atlas');
      await expect(
        service.request({ action: 'agent/run', params: { id: 'atlas', prompt: 'Work' } }),
      ).rejects.toThrow();
  ...
  it('dispatches only active agents, sequentially, and creates reviewable feedback for Learning agents', async () => {
    ...
    await service.request({
      ...
      params: { title: 'An accessible dialog', brief: 'Verify keyboard focus and labels.' },
    });
    ...
    await service.request({
      ...
      params: { id: deliverable.id, feedback: 'Always return focus to the trigger.' },
    });
  ...
  it('deactivation stops the owned process, cancels queued work, and excludes future dispatch', async () => {
    ...
    await expect(
      service.request({ action: 'agent/run', params: { id: 'atlas', prompt: 'Another task' } }),
  ...
  it('does not write through a workspace junction or create descendants outside the workspace', async () => {
    const { service, root } = await setup();
    const outside = await mkdtemp(path.join(os.tmpdir(), 'meridian-workbench-outside-'));
    await symlink(
      outside,
      path.join(root, '.meridian'),
      process.platform === 'win32' ? 'junction' : 'dir',
    );
    await expect(save(service, 'atlas')).rejects.toThrow('inside the workspace');
    expect(await readdir(outside)).toEqual([]);
  });
  it('refuses edits after the workspace changes or closes', async () => {
    const original = await mkdtemp(path.join(os.tmpdir(), 'meridian-workbench-context-'));
  ...
  it.each(['sidecar disconnected', 'governance halt'])(
    ...
    async (cause) => {
      ...
      const stop = vi.fn();
      const { service, snapshot } = await setup({
        sidecar: () => (connected ? { request: async () => ({ entries: [] }) } : undefined),
        launcher: (agent) => ({
          adapterId: agent.id,
          pid: undefined,
          start: async () => ({ protocolVersion: 1, agentCapabilities: {}, authMethods: [] }),
      ...
      await save(service, 'atlas');
      await activate(service, 'atlas');
      await service.request({ action: 'agent/run', params: { id: 'atlas', prompt: 'Review' } });
      await until(async () => prompted);
      if (cause === 'sidecar disconnected') {
        connected = false;
        await service.reconcile();
      } else expect(hostedSessionRegistry.halt('governed-session', 'Gate rejected')).toBe(true);
      await until(async () => (await snapshot()).runs[0].state === 'cancelled');
      expect(stop).toHaveBeenCalled();
      await until(async () => !hostedSessionRegistry.ids().includes('governed-session'));
    },
  );
  it('withholds a granted permission when its audit record fails', async () => {
    let decision: unknown;
    const humanApprover = vi.fn(async () => ({ outcome: 'selected' as const, optionId: 'allow' }));
    const { service, root, snapshot } = await setup({
      humanApprover,
      sidecar: () => ({
        request: async (method) => {
          if (method === 'acp/permissionDecision') throw new Error('Ledger unavailable');
          return { entries: [] };
        },
      }),
      launcher: (agent, options) => ({
    ...
    await save(service, 'atlas');
    await activate(service, 'atlas');
    await service.request({ action: 'agent/run', params: { id: 'atlas', prompt: 'Inspect' } });
  ...
  it.each(['refusal', 'max_tokens', 'max_turn_requests'])(
    ...
    async (reason) => {
      ...
      await until(async () => (await snapshot()).deliverables[0].state === 'failed');
      expect((await snapshot()).runs[0].error).toContain(reason);
      await expect(
        service.request({
          action: 'deliverable/complete',
          params: { id: (await snapshot()).deliverables[0].id, feedback: 'Accept' },
        }),
  ...
  it('runs an independent ACP subprocess through real stdio, permission approval, output streaming, and persistence', async () => {
    ...
    await service.request({
      action: 'agent/run',
      params: { id: 'wire-agent', prompt: 'Run the fixture task independently.' },
    });
    await vi.waitFor(async () => expect((await snapshot()).runs[0].state).toBe('completed'), {
      timeout: 15_000,
      interval: 100,
    });
    expect((await snapshot()).runs[0].output).toContain('Working on it.Done.');
    expect(await readFile(path.join(root, 'output.txt'), 'utf8')).toContain('real workspace input');
    expect(decisions.length).toBeGreaterThan(0);
    expect(
      JSON.parse(await readFile(path.join(root, '.meridian/workbench/state.json'), 'utf8')).runs[0]
        .state,
    ).toBe('completed');
  }, 25_000);
});
