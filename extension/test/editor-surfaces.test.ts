...
describe("editor provenance integrity", () => {
  ...
  it("uses committed authorship only when the whole working copy matches HEAD", () => {
    ...
    expect(
      matchingCommittedLine("inserted\nfirst\nsecond\n", lines, 2),
    ).toBeUndefined();
    expect(
      matchingCommittedLine("changed\nsecond\n", lines, 2),
    ).toBeUndefined();
  ...
  it("allows only canonical paths inside the current workspace, including junction resolution", async () => {
    ...
    await writeFile(path.join(outside, "other.ts"), "outside");
    expect(await resolveWorkspaceSource(root, "source.ts")).toBe(
      await realpath(path.join(root, "source.ts")),
    );
    await expect(
      resolveWorkspaceSource(root, "../outside/other.ts"),
    ...
    await expect(
      resolveWorkspaceSource(root, "linked/other.ts"),
    ).rejects.toThrow(/inside/);
  });
});
