...
// Python keywords that cannot appear as TypedDict class-body annotations;
// defs carrying one are emitted in the functional TypedDict form instead.
  ...
  for (const [file, schema] of schemas) {
    for (const [name, def] of Object.entries(schema.$defs ?? {})) {
      ...
      if ((def.type === 'object' && def.properties && Object.keys(def.properties).length > 0) || isClosedEmptyObject) {
        ...
        // Property names that collide with Python keywords (e.g. `class`)
        // cannot ride the class-body syntax; the whole def falls back to
        // the equivalent functional TypedDict form, keys quoted.
        const hasKeywordKey = entries.some(([key]) => PY_KEYWORDS.has(key));
        if (hasKeywordKey) {
          ...
          for (const [key, prop] of entries) {
            ...
            lines.push(`    ${JSON.stringify(key)}: ${wrapped},`);
          }
          lines.push('})');
        } else {
