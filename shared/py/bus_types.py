...
class InteropConflictsParams(TypedDict):
    repoPath: NotRequired[str]
    ref: NotRequired[str]  # Where the walk starts. Default HEAD.
    maxCommits: NotRequired[int]  # How many commits to walk, 1 to 5000. Default 200.
    record: NotRequired[bool]  # Append each disagreement not already recorded to the signed ledger, as digests.

class InteropClaim(TypedDict):
    claimant: str  # Who makes the statement: a notes ref, a trailer key, meridian-ledger, or commit-author.
    tool: str  # The vendor id of the claimant, for the vendor tag.
    agents: list[str]  # The agents the claimant attributes the commit to, canonically named.
    confidence: ObservationConfidence
    evidence: str  # Digests of the records, or the ledger range. Never the content of a record.

class InteropDisagreement(TypedDict):
    commit: str
    kind: Literal["conflicting", "incomplete"]  # conflicting when two claims share no agent; incomplete when every pair overlaps but the claims still differ.
    claims: list[InteropClaim]
    digest: str  # Stable across reads, so the same disagreement is recorded once.
    notResolved: str  # The sentence saying no claim is preferred.

class InteropConflictsResult(TypedDict):
    examined: int
    truncated: bool  # The walk stopped at maxCommits. A clean report over part of history is not a clean report.
    claimed: int  # Commits with at least one claim.
    agreeing: int  # Commits with two or more claims that agree.
    disagreements: list[InteropDisagreement]
    recorded: int  # Disagreements newly appended to the ledger. Zero unless record was set.

class InteropExportParams(TypedDict):
    repoPath: NotRequired[str]
    format: Literal["attribution-json", "git-notes"]
    ref: NotRequired[str]  # Default HEAD.
    paths: NotRequired[list[str]]  # attribution-json only: the files to export. Default every tracked file, bounded.
    maxCommits: NotRequired[int]  # git-notes only: how many commits from ref, 1 to 5000. Default 200.
    write: NotRequired[bool]  # git-notes only: write the notes. Without it nothing is written.
    installedAt: NotRequired[str | float]  # Meridian's installation time. Older commits are attributed at inferred, never observed (FR-M41-16).

class InteropNotesExport(TypedDict):
    ref: str
    commits: int
    truncated: bool
    toWrite: int  # Notes that are absent or would change.
    unchanged: int  # Notes already saying the same thing, left alone.
    written: int  # Zero unless write was true.

class InteropExportResult(TypedDict):
    format: Literal["attribution-json", "git-notes"]
    document: NotRequired[dict[str, Any]]  # attribution-json: the meridian-loom/attribution-export@1 document.
    notes: NotRequired[InteropNotesExport]
...
class EvidenceGateResult(TypedDict):
    preregistration: NotRequired[dict[str, Any]]  # The thresholds digest applied, the digest the study registered, and whether the preregistration is intact (docs/evidence-gate.md section 5). invalidated is true when it is not.
    study: NotRequired[dict[str, Any]]  # Whether a study record was supplied and valid, its problems, the FR-M46-14 fields it does not record, and its exclusions.
    context: NotRequired[dict[str, Any]]  # Ledger facts shown beside the measures and never deciding them: gate evaluations and stops, the FR-M20-06 hygiene signals, and gate-to-approval latency.
    firstValue: NotRequired[dict[str, Any]]  # FR-M46-04, scored beside the rule: five onboarding sessions and ten review tasks, each met, not_met or unmeasured.
    scope: dict[str, Any]  # The echoed scope and the supplied baseline.
    stories: dict[str, Any]  # total / required / byArm counts, the arm C stories allocated but absent from the ledger, and the story ids in scope.
    measures: dict[str, Any]  # The ten preregistered measures P1–P6, C1–C3 and O1, each {id, status, value, threshold, source, note, ...}. status is met, not_met or unmeasured — never a zero standing in for absent evidence, and unmeasured is never satisfied.
...
class PortabilityTrustParams(TypedDict):
    fingerprint: str
    humanApproved: bool
...
CAPABILITIES: tuple[CapabilityDefinition, ...] = (
    ...
    {"id": "orchestra.loops", "tier": "orchestra", "description": "The six canonical loops (FR-M4-03) and their gates, checkpoints, resume and replay (F3).", "rpcMethods": ["loop.start", "loop.stop", "loop.status", "loop.resume", "loop.replay"]},
    {"id": "orchestra.routing", "tier": "orchestra", "description": "Model router behind the deterministic engine (FR-M8-15..17).", "rpcMethods": ["router/requestModelCall", "router/dependencyRatio"]},
    {"id": "orchestra.tools", "tier": "orchestra", "description": "Permissioned native tool surface (FR-M9-02/03).", "rpcMethods": ["tools/invoke"]},
    {"id": "orchestra.memory", "tier": "orchestra", "description": "Memory fabric and layered procedural memory (FR-M7).", "rpcMethods": ["memory/retrieve", "memory/write", "memory/layered"]},
    {"id": "orchestra.comprehension", "tier": "orchestra", "description": "Brownfield comprehension records and the AC-35 gate (FR-M38).", "rpcMethods": ["comprehension/record", "comprehension/gate"]},
    {"id": "orchestra.adapters", "tier": "orchestra", "description": "Adapter discovery, hot plug, promotion (FR-M31, FR-M5-05).", "rpcMethods": ["adapters/discover", "adapters/plug", "adapters/unplug", "adapters/promote"]},
    {"id": "governor.decisions", "tier": "governor", "description": "Decision records, counterfactual replay, explanation-free gates (FR-M13).", "rpcMethods": ["decisions/record", "decisions/ablate", "decisions/gate"]},
    {"id": "governor.portability", "tier": "governor", "description": "Signed adapter package export/import/diff (FR-M16).", "rpcMethods": ["portability/export", "portability/import", "portability/diff", "portability/trust"]},
    {"id": "governor.trainer", "tier": "governor", "description": "Trainer runs and human-approved promotion (FR-M14).", "rpcMethods": ["trainer/train", "trainer/promote", "trainer/rollback"]},
    {"id": "governor.tenancy", "tier": "governor", "description": "Tenant registration and the story queue (SEC-23, FR-M21).", "rpcMethods": ["tenancy/register", "queue/enqueue", "queue/tick"]},
    {"id": "recorder.annotations", "tier": "flight-recorder", "description": "Ledger annotations/bookmarks and agent-detected issues (FR-M10-16, FR-M24-05).", "rpcMethods": ["annotations/add", "issues/record"]},
