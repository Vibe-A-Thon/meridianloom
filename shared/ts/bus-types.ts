...
export interface EnforcementPointsParams {
  /** One control id. Omit for the whole registry. An unknown id is an error, never an empty result — a control nobody declared is a bug, not an absence (P26). */
  "control"?: string;
  /** Whether an SCM-native check is configured for this workspace. Default false, which is v1's reality (D37): with no binding, scm-point controls report their effective sidecar point instead. */
  "scmBindingConfigured"?: boolean;
}

export interface EnforcementPointsResult {
  /** Bumped whenever the vocabulary or a declaration's semantics change, so a recorded declaration stays interpretable without this build. */
  "vocabularyVersion": string;
  /** The closed FR-M42-11 enforcement-point vocabulary. */
  "vocabulary": string[];
  "scmBindingConfigured": boolean;
  /** Control id to its effective declaration. */
  "controls": Record<string, unknown>;
}

export interface ControlDeclaration {
  "control": string;
  /** Where the control ACTUALLY binds, after any D37 downgrade. */
  "enforcementPoint": "editor" | "extension_host" | "sidecar" | "scm" | "ci" | "advisory_only";
  /** false = advisory: it observes, records and warns, and no surface may render it as enforced (SEC-32). */
  "enforced": boolean;
  "vocabularyVersion": string;
  /** What could bypass this control and who could do so. Written to be quoted into an audit bundle verbatim (FR-M42-12). */
  "boundaryNote": string;
}

export interface RunPreflightParams {
  /** FR-M40-02: which door this run came through. Closed vocabulary, identical to the ledger CHECK constraint carried since schema v2. */
  "origin": "ui" | "command" | "omnibar" | "chat" | "editor" | "file" | "connector" | "api";
  /** What the human asked for, in their words. */
  "intent": string;
  "repo": string;
  "baseBranch"?: string;
  /** Role to adapter id. Which agent fills which role for THIS run. */
  "adapters"?: Record<string, unknown>;
  "mode"?: "dry_run" | "live";
  "gates"?: string[];
  /** null is unknown; 0 is a measured zero (P26). */
  "estimateUsd"?: number | null;
  "costCeilingUsd"?: number | null;
  /** Supplied only to re-preflight an existing request. */
  "runId"?: string;
}
...
export interface RunPreflight {
  ...
  "repo": string;
  "baseBranch": string;
  /** Derived from the run id, never supplied — two doors passing different branches for one run is the divergence the single contract prevents. */
  "branch": string;
  /** The path that WOULD be created. Nothing exists yet. */
  "worktree": string;
  "estimateUsd"?: number | null;
  "costCeilingUsd"?: number | null;
  "gates": string[];
  "mode": "dry_run" | "live";
  /** false when any of FR-M40-03's six answers is missing. */
  "confirmable": boolean;
  "missing": string[];
}
...
export interface RunStartParams {
  "preflight": RunPreflight;
  /** The human's confirmation of THIS preflight. false is refused. */
  "confirmed": boolean;
  /** FR-M40-05: the launching role, checked against the role pack; absent uses the pack's defaultRole. A read-only role may start a dry run and not a live one. */
  "role"?: string;
  /** FR-M42-05: when set, a git-asserted identity is refused. An asserted identity never satisfies a verified requirement. */
  "requiresVerifiedIdentity"?: boolean;
  /** FR-M18-07: the agent identity the run worktree's commits are attributed to. Absent uses the Developer role's adapter, else the first role by name. */
  "adapterId"?: string;
  "rolePath"?: string;
  "policyPath"?: string;
  /** Repository root. Absent means the handshake workspaceDir. */
...
export interface RunStartResult {
  ...
  "mode": string;
  "authorisedBy": string;
  /** The assurance level of the authorising identity, so a run authorised by a git name is never later read as verified. */
  "assurance": string;
  /** The run's first ledger entry. */
...
export interface ForeignRecord {
  /** Which tool wrote it. `unknown-tool` for a notes ref Meridian does not recognise — named as unknown rather than guessed. */
  "tool": string;
  "kind": "git-note" | "trailer";
  /** The notes ref, or the trailer key. */
  "source": string;
  "commit": string;
  /** sha256 over the exact bytes read, before any parsing and with no normalisation. */
  "digest": string;
  "observedAt": string;
  /** FR-M52-02: a foreign record is never above `inferred`. The enum is closed to one value so no future change can widen it without changing this contract. */
  "confidence": "inferred";
  "truncated": boolean;
  /** SEC-43, carried on every record so it cannot be lost between the ledger and a surface. */
  "notEndorsed": string;
...
export interface ForeignPayload {
  "format": "json" | "text";
  "parsed": boolean;
  /** Scalar leaves only. A nested structure from an untrusted file is where a surface renders something nobody designed for. */
  "fields"?: Record<string, unknown>;
}

export interface InteropRecordsParams {
  "repoPath"?: string;
  /** Also read session-log and vendor co-author trailers from this commit. */
  "commit"?: string;
}

export interface InteropRecordsResult {
  "records": ForeignRecord[];
}

export interface InteropNotariseParams {
  "repoPath"?: string;
  "commit"?: string;
}

export interface InteropNotariseResult {
  /** How many records were newly recorded. */
  "notarised": number;
  /** Unchanged records already in the ledger. Re-recording them would prove nothing twice. */
  "alreadyNotarised": number;
  "records": ForeignRecord[];
}

export interface InteropVerifyParams {
  "repoPath"?: string;
  "commit"?: string;
}

export interface InteropVerdict {
  "ok": boolean;
  "tool": string;
  "source": string;
  "commit": string;
  "digestAtNotarisation": string;
  /** null when the record is no longer in the repository — gone, which is not the same as altered. */
  "digestNow"?: string | null;
  "detail": string;
}

export interface InteropVerifyResult {
  "verdicts": InteropVerdict[];
  "altered": number;
  "missing": number;
  /** Notarisation entries whose recorded detail could not be read back because the per-subject blob key was destroyed. Reported rather than skipped: an entry that cannot be checked is not an entry that passed. */
  "unreadable": number;
}

export interface InteropConflictsParams {
  "repoPath"?: string;
  /** Where the walk starts. Default HEAD. */
  "ref"?: string;
  /** How many commits to walk, 1 to 5000. Default 200. */
  "maxCommits"?: number;
  /** Append each disagreement not already recorded to the signed ledger, as digests. */
  "record"?: boolean;
}

export interface InteropClaim {
  /** Who makes the statement: a notes ref, a trailer key, meridian-ledger, or commit-author. */
  "claimant": string;
  /** The vendor id of the claimant, for the vendor tag. */
  "tool": string;
  /** The agents the claimant attributes the commit to, canonically named. */
  "agents": string[];
  "confidence": ObservationConfidence;
  /** Digests of the records, or the ledger range. Never the content of a record. */
  "evidence": string;
}

export interface InteropDisagreement {
  "commit": string;
  /** conflicting when two claims share no agent; incomplete when every pair overlaps but the claims still differ. */
  "kind": "conflicting" | "incomplete";
  "claims": InteropClaim[];
  /** Stable across reads, so the same disagreement is recorded once. */
  "digest": string;
  /** The sentence saying no claim is preferred. */
  "notResolved": string;
}

export interface InteropConflictsResult {
  "examined": number;
  /** The walk stopped at maxCommits. A clean report over part of history is not a clean report. */
  "truncated": boolean;
  /** Commits with at least one claim. */
  "claimed": number;
  /** Commits with two or more claims that agree. */
  "agreeing": number;
  "disagreements": InteropDisagreement[];
  /** Disagreements newly appended to the ledger. Zero unless record was set. */
  "recorded": number;
}

export interface InteropExportParams {
  "repoPath"?: string;
  "format": "attribution-json" | "git-notes";
  /** Default HEAD. */
  "ref"?: string;
  /** attribution-json only: the files to export. Default every tracked file, bounded. */
  "paths"?: string[];
  /** git-notes only: how many commits from ref, 1 to 5000. Default 200. */
  "maxCommits"?: number;
  /** git-notes only: write the notes. Without it nothing is written. */
  "write"?: boolean;
  /** Meridian's installation time. Older commits are attributed at inferred, never observed (FR-M41-16). */
  "installedAt"?: string | number;
}

export interface InteropNotesExport {
  "ref": string;
  "commits": number;
  "truncated": boolean;
  /** Notes that are absent or would change. */
  "toWrite": number;
  /** Notes already saying the same thing, left alone. */
  "unchanged": number;
  /** Zero unless write was true. */
  "written": number;
}

export interface InteropExportResult {
  "format": "attribution-json" | "git-notes";
  /** attribution-json: the meridian-loom/attribution-export@1 document. */
  "document"?: Record<string, unknown>;
  "notes"?: InteropNotesExport;
}
...
export interface EvidenceGateParams {
  "fromSequence"?: number;
  "toSequence"?: number;
  /** The team's own pre-Meridian change failure rate. The ledger cannot know it; absent, the comparison reads unavailable rather than inventing a baseline to beat. */
  "baselineChangeFailureRate"?: number;
}

export interface EvidenceGateResult {
  /** The thresholds digest applied, the digest the study registered, and whether the preregistration is intact (docs/evidence-gate.md section 5). invalidated is true when it is not. */
  "preregistration"?: Record<string, unknown>;
  /** Whether a study record was supplied and valid, its problems, the FR-M46-14 fields it does not record, and its exclusions. */
  "study"?: Record<string, unknown>;
  /** Ledger facts shown beside the measures and never deciding them: gate evaluations and stops, the FR-M20-06 hygiene signals, and gate-to-approval latency. */
  "context"?: Record<string, unknown>;
  /** FR-M46-04, scored beside the rule: five onboarding sessions and ten review tasks, each met, not_met or unmeasured. */
  "firstValue"?: Record<string, unknown>;
  /** The echoed scope and the supplied baseline. */
  "scope": Record<string, unknown>;
  /** total / required / byArm counts, the arm C stories allocated but absent from the ledger, and the story ids in scope. */
  "stories": Record<string, unknown>;
  /** The ten preregistered measures P1–P6, C1–C3 and O1, each {id, status, value, threshold, source, note, ...}. status is met, not_met or unmeasured — never a zero standing in for absent evidence, and unmeasured is never satisfied. */
  "measures": Record<string, unknown>;
  /** {verdict, reasons, invalidated, undecidedOutcome?, unmeasured?}. verdict is go | stop | pivot | kill | insufficient_evidence | unclassified, applying docs/evidence-gate.md §4 in order over three-valued logic. invalidated is true when the preregistration is not intact (§5). */
...
/** Explicit, human-approved trust for an external package signer fingerprint (audit TASK-330). */
export interface PortabilityTrustParams {
  "fingerprint": string;
  "humanApproved": boolean;
}

/** Trust record outcome. */
export interface PortabilityTrustResult {
  "trusted": boolean;
}
...
export interface LicenceSummary {
  ...
  "trial"?: boolean;
  "boundDevelopers"?: number;
  "boundMachines"?: number;
}
...
export interface LicenceStatusResult {
  ...
  "state": "none" | "valid" | "grace" | "expired" | "not-yet-valid" | "invalid" | "untrusted-key" | "wrong-machine" | "wrong-developer";
  "reason": string;
  "premiumActive": boolean;
  "licence"?: LicenceSummary;
  /** The licence file that decided the state. */
  "source"?: string;
  /** Days to expiry; negative once expired. Absent for a perpetual licence. */
  "daysRemaining"?: number;
  /** Every licence file examined and its state. */
  "checked"?: string[];
  /** This machine's fingerprint (MLM1-...) for ordering a per-machine licence; null when the OS offers no machine id. */
  "machineFingerprint"?: string | null;
}

export interface LicenceStatusParams {
  /** Re-read the licence files instead of using the cached evaluation. */
  "reload"?: boolean;
  /** Also return this machine's fingerprint. */
  "includeFingerprint"?: boolean;
}

export interface LicenceInstallParams {
  /** Absolute path of a .mlic licence file. */
  "path"?: string;
  /** The licence file's contents (alternative to path). */
  "text"?: string;
  /** 'user' (default) for a per-developer licence; 'machine' for a licence every account on the host shares (needs administrator rights). */
  "scope"?: "user" | "machine";
}

export interface LicenceRemoveParams {
  "scope"?: "user" | "machine";
}

export interface LicenceRemoveResult {
  "removed": string[];
...
export interface MethodMap {
  ...
  "interop/conflicts": { params: InteropConflictsParams; result: InteropConflictsResult };
  "interop/export": { params: InteropExportParams; result: InteropExportResult };
