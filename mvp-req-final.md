...
| **MVP-R1.6** | `FR-M46-06` — a tested support matrix of operating system, remote configuration, Python version and editor version SHALL be published with the release. | **`MVP-GAP`** |
| **MVP-R1.7** | `FR-M46-07` — upgrade failure, disk exhaustion, sidecar crash, corrupted bundle and restore SHALL be rehearsed on three desktop platforms plus one remote configuration, with zero loss of acknowledged ledger entries. | **`MVP-GAP`** |
...
| `NFR-43` Soak stability, 7 days | `futures_requirements.md` | **`MVP-GAP`** (`MVP-R6.4`) |
...
| `NFR-48` **AI-BOM freshness** — the published bill of materials is generated by the build, never hand-maintained, and drift fails the build | §16 (`M50`) | **`MVP-GAP`** (`MVP-R7.3`) |
...
| `SEC-42` A third-party provenance record is **untrusted input**: parsed under the existing allow-list, never executed, never promoted above `inferred` | §16 (`M52`) | **`MVP-GAP`** (`MVP-R7.1`) |
| `SEC-43` Notarising a third-party record SHALL NOT import that tool's trust: the digest is signed, the content is not vouched for | §16 (`M52`) | **`MVP-GAP`** (`MVP-R7.1`) |
| `SEC-44` The AI-BOM SHALL NOT disclose workspace content, credentials or customer identifiers — only what the package itself ships | §16 (`M50`) | **`MVP-GAP`** (`MVP-R7.3`) |
...
| `AC-62` **The AI-BOM matches the package.** Every agent, skill, instruction document and runtime preset in the VSIX appears in the published CycloneDX AI-BOM with a matching digest; a deliberate mismatch fails the build | §16 (`M50`) | **`MVP-GAP`** (`MVP-R7.3`) |
