...
- **D33 (closed, with follow-up).** FR-M39-02 pause mechanism: the sidecar records a durable `spend_ceiling` ledger entry (FR-M10-08) and dispatches a `spend/ceiling` notification; the extension host pauses the hosted ACP session at its next checkpoint (it owns the ACP wire, same split as `gate.halt`). Sidecar half implemented (3608ecd); **host-side pause handling is open** and lands with the extension/ACP-host work before F1 exit.
