...
- **Current task:** F1 — T26 cost bound to gate decision + merged commit, merged vs abandoned separation, 20-change reconciliation (FR-M45-01/02, AC-51). NOTE: no FR-M45 code exists in core yet — greenfield module, own files only (metrics/economics.py or similar + tests); spend.py/pricing.py/budget.py cover the panel but not gate-bound cost.
