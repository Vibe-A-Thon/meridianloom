...
- **Current workstream:** N2 Workstream C — Bypass detection and policy distribution (T09–T12); N2 A+B done
- **Current task:** T09 five named circumventions detection + T10 ledger entry + coverage downgrade + T11 signed policy bundles + T12 policy simulator
- **Last commit:** f7a6ff1 — N2-B done: requiresVerifiedIdentity (global + per-branch), ledger-recorded revocation list (immediate in-process, 5min outer envelope), approvals re-checked at gate execution incl. pre-revocation approvals, identity.revoke RPC; 19 new + sweeps green.
